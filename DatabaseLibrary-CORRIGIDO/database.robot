*** Settings ***
Documentation   [14/10/2019] https://marketsquare.github.io/Robotframework-Database-Library/#Check%20If%20Exists%20In%20Database
...
Exemplo 01: Executando scripts SQL via Arquivos
    # Execute SQL String                                  sql_string=DROP TABLE IF EXISTS account
...
Conectar no Banco de Dados
    ## PostgreSQL
    Connect To Database   db_module=psycopg2   db_name=robot   db_user=postgres   db_password=1234   db_host=localhost   db_port=5432
    ## SQLite 3
...
Confere se "${USER}" foi inserido com sucesso
    # Verifica se existe exatamente 1 registro com o username especificado
    ${count}=    Row Count    select_statement=SELECT username FROM account WHERE username = '${USER}'
    Should Be Equal As Integers    ${count}    1    Registro para o usuário '${USER}' não encontrado
